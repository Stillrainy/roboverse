...
import roboverse
from roboverse.agents import OnlineAgentTrainer


def parse_args():
    parser = argparse.ArgumentParser(description='Train online agent')
    parser.add_argument('-e', '--env-id', type=str, default='Widow250DrawerRandomizedPickPlace-v0',
...
if __name__ == "__main__":
    ...
    if args.log_dir is None:
        save_dir = os.path.join(
            os.getcwd(),
            'logs',
            f'online_{datetime.now().strftime("%Y%m%d_%H%M%S")}')
    else:
        save_dir = args.log_dir
    os.makedirs(save_dir, exist_ok=True)

    trainer = OnlineAgentTrainer(
        env_id=args.env_id,
